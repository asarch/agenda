...
class EditorNotas:
   def __init__(self):
      ...
      # *** ERROR ***
      # Aqui esta presente el error del vecto de Caveman2, que pasa si
      # los id's no son consecutivos, no se podra seleccionar el tipo
      # de nota correcto (de hecho ya tuvimos ese error por no ordenar
      # por id del tipo de nota la consulta).
      for instance in session.query(TipoNota).order_by(TipoNota.id):
